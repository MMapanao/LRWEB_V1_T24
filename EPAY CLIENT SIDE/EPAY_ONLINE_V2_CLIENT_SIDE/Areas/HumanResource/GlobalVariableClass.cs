...
namespace LRWEB_V1_CLIENT_SIDE_T24.Areas.HumanResource
{
    public class GlobalVariableClass
    {
        ...
        public static bool Authenticated(string myticketsession, string mycredsession)
        {
			...
			var obj = conn.Query<_Login>(@"Select * from LRWEB_V1_AUTH where
			EmployeeEmail=@email and 
			token=@token and
			convert(BIGINT,timeexpiration) >= convert(BIGINT,@timenow )
			and convert(BIGINT,timestamp) <= convert(BIGINT,@timenow) 
			AND logout ='0' and groupcode=@groupcode", new
			{
				email = arrcred[6],
